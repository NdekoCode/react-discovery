...
export const CounterContextProvider:FunctionComponent<Props> = memo(({children,start=0,step=1})=>{
    const [counter,setCounter] = useState<number>(start);

    const increment =useCallback( ()=>setCounter(n=>n+step),[step]);
    const decrement = useCallback(()=>setCounter(n=>n>step? n-step:0),[step]);

    const value = useMemo(()=>({counter,increment,decrement}),[counter,increment,decrement]);
